...
# get info on water/steam injected by type in each field -----
  ...
  type_field = inj_single[, .(type_inj = sum(SteamWaterInjected, na.rm = T)), by = .(FieldCode, FieldName, well_type_name)] 
  type_field[is.na(type_inj), type_inj := 0]
  type_field[, total_inj := sum(type_inj, na.rm = T), by = .(FieldCode, FieldName)]
  type_field[, perc := type_inj/sum(type_inj, na.rm = T), by = .(FieldCode, FieldName)]
  type_field = type_field[order(-total_inj,-type_inj)]

# get info on injection by type in each, over the years -----
  
  type_field_yr = inj_single[, .(type_inj = sum(SteamWaterInjected, na.rm = T)), by = .(FieldCode, FieldName, year, well_type_name)] 
  type_field_yr[is.na(type_inj), type_inj := 0]
  type_field_yr[, total_inj_year := sum(type_inj, na.rm = T), by = .(FieldCode, FieldName, year)]
  type_field_yr[, perc := type_inj/sum(type_inj, na.rm = T), by = .(FieldCode, FieldName, year)]
  type_field_yr = type_field_yr[order(FieldName,year,-type_inj)]
  
  type_field_yr = type_field_yr[ci_info, on = c('FieldName' = 'field_name'), nomatch = 0]
  labs_field_yr = unique(type_field_yr[, c('FieldCode', 'FieldName', 'year', 'total_inj_year', 'ci_2015')])
  
# 2018 only -----
  
  type_field_2018 = inj_single[ year == 2018, .(type_inj = sum(SteamWaterInjected, na.rm = T)), by = .(FieldCode, FieldName, well_type_name)] 
  type_field_2018[is.na(type_inj), type_inj := 0]
  type_field_2018[, total_inj := sum(type_inj, na.rm = T), by = .(FieldCode, FieldName)]
  type_field_2018[, perc := type_inj/sum(type_inj, na.rm = T), by = .(FieldCode, FieldName)]
  type_field_2018 = type_field_2018[order(-total_inj,-type_inj)]
  
# 2015 only -----
  
  type_field_2015 = inj_single[ year == 2015, .(type_inj = sum(SteamWaterInjected, na.rm = T)), by = .(FieldCode, FieldName, well_type_name)] 
  type_field_2015[is.na(type_inj), type_inj := 0]
  type_field_2015[, total_inj := sum(type_inj, na.rm = T), by = .(FieldCode, FieldName)]
  type_field_2015[, perc := type_inj/sum(type_inj, na.rm = T), by = .(FieldCode, FieldName)]
...
# plot top 10 injection fields -----
  ...
  top10inj = unique(type_field[order(-total_inj), FieldName])[1:10]
  ...
  bar_top10inj = ggplot(type_field_yr[ year %in% c(2015,2018) & FieldName %in% top10inj], 
                        aes(x = FieldName, y = type_inj/1e6)) + geom_bar(stat = 'identity', aes(fill = well_type_name)) +
    ...
    geom_text(data = labs_field_yr[year %in% c(2015,2018) & FieldName %in% top10inj], 
              aes(x = FieldName, y = (total_inj_year/1e6) + 30, label = paste0(ci_2015, ' g/MJ')),
  ...
  bar_top10prod = ggplot(type_field_yr[ year %in% c(2015,2018) & FieldName %in% top10prod], 
                         aes(x = FieldName, y = type_inj/1e6)) + geom_bar(stat = 'identity', aes(fill = well_type_name)) +
    ...
    facet_wrap(vars(year), nrow = 2) +
    scale_fill_manual(values = prim_calepa_pal[c(1,3,2)]) + 
    geom_text(data = labs_field_yr[year %in% c(2015,2018) & FieldName %in% top10prod], 
              aes(x = FieldName, y = (total_inj_year/1e6) + 30, label = paste0(ci_2015, ' g/MJ')),
              size = 5.5, fontface = 'plain', family = 'Secca Soft') +
    theme_line
  # bar_top10prod
...
# export to csv files ----
  ...
  fwrite(type_field, paste0(data_dir, 'injection-by-well-type-per-field_1977-2018.csv'), row.names = F)
  fwrite(type_field_yr, paste0(data_dir, 'injection-by-well-type-per-field-per-year_1977-2018.csv'), row.names = F)
