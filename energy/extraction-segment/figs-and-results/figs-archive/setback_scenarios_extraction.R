# extraction production and ghg emissions under setback scenarios
...
# reorder factor levels -------
  ...
  dt_res[, setback_scenario := factor(setback_scenario, levels = rev(c('setback_5280ft', 'setback_2500ft', 'setback_1000ft', 'no_setback')))]
  dt_res[, prod_quota_scenario := factor(prod_quota_scenario, levels = c('no quota', 'quota_10', 'quota_20', 'quota_40', 'quota_00'))]
...
# ------------------------- plots ------------------------- 
  ...
  # plot: extraction under price paths -------- 
    ...
    fig_setback_bbl = ggplot(dt_res, aes(x = year, y = prod_mbbl, color = setback_scenario)) +
      geom_line() +
      labs(title = NULL,
           subtitle = NULL, 
           x = NULL,
      ...
      scale_color_manual(values = ucsb_distinct,
                         labels = c('setback_5280ft' = '5,280ft setback', 
                                    'setback_2500ft' = '2,500ft setback',
                                    'setback_1000ft' = '1,000ft setback', 
                                    'no_setback' = "No setback")) + 
      theme_line +
      theme(legend.position = "top")
    fig_setback_bbl
    
    ggsave(fig_setback_bbl,
           filename = file.path(fig_path, 'extraction_setback_scenarios_production.png'),
           width = 6.5,
           height = 4.5,
           dpi = 400, 
           units = 'in', 
    ...
    ggsave(fig_setback_bbl,
           filename = file.path(fig_path, 'extraction_setback_scenarios_production.pdf'),
           width = 6.5,
           height = 4.5,
           units = 'in', 
           device = 'pdf')
    
    embed_fonts(file.path(fig_path, 'extraction_setback_scenarios_production.pdf'),
                outfile = file.path(fig_path, 'extraction_setback_scenarios_production.pdf'))
    
    
  # plot: ghg emissions under price paths -------- 
    
    fig_setback_ghg = ggplot(dt_res, aes(x = year, y = ghg_mmt_inno_ccs_adj, color = setback_scenario)) +
      geom_line() +
      labs(title = 'Greenhouse gas emissions under setback scenarios',
    ...
    ggsave(fig_setback_ghg,
           filename = file.path(fig_path, 'extraction_setback_scenarios_ghg.png'),
           width = 6.5,
           height = 4.5,
           dpi = 400, 
           units = 'in', 
    ...
    ggsave(fig_setback_ghg,
           filename = file.path(fig_path, 'extraction_setback_scenarios_ghg.pdf'),
           width = 6.5,
           height = 4.5,
           units = 'in', 
           device = 'pdf')
    
    embed_fonts(file.path(fig_path, 'extraction_setback_scenarios_ghg.pdf'),
                outfile = file.path(fig_path, 'extraction_setback_scenarios_ghg.pdf'))
