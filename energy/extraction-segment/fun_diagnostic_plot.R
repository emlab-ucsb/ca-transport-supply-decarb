plot_diagnostic_outputs <- function(oil_price_selection, output_extraction) {
  ...
  state_base = fread(file.path(base_path, 'extraction_2021-07-21', 'revised-new-wells-setback', 'diagnostic-state-level-results.csv'), header = T)
  
  # add column for base case
  state_base[, version := "adjust-new-wells-setback"]
