...
calc_2045_ghg <- function(inputs, scen) {
  ...
  dt_info_z = unique(inputs[year == 2020])
  ...
  setkey(inputs, doc_field_code, oil_price_scenario, innovation_scenario, carbon_price_scenario, ccs_scenario, 
         setback_scenario, setback_existing, prod_quota_scenario, excise_tax_scenario, year)
  ...
  for (i in seq_along(pred_years)) {
    ...
    if(t < max(pred_years)) {
      ...
      ## filter for t + 1
      info_next_year = unique(inputs[year == t + 1])
  ...
  ## filter for 2045, total ghg
  target_ghg <- state_all %>%
    filter(year == 2045) %>%
    select(total_ghg_mtCO2e) %>%
    as.numeric()
  
  return(target_ghg)
  
  
  rm(vintage_all, state_all, field_all, existing_prod_dt, new_prod_dt, scen, inputs, 
     depl_2019_z, prod_2019_z, trr_2020_z, depl_2020_z, dt_depl_z, dt_info_z,  
     scenario_name_z)
