...
load_scenarios_dt_full = function(scenario_selection, scenario_z) {
  # inputs -----
  ...
  scenario_dt = scenario_z[oilpx_scens, on = .(oil_price_scenario), allow.cartesian = T, nomatch = 0]
  scenario_dt = scenario_dt[vars_dt, on = .(year), allow.cartesian = T, nomatch = 0]
  scenario_dt = scenario_dt[innovation_scens, on = .(year, innovation_scenario), nomatch = 0]
  scenario_dt = scenario_dt[carbonpx_scens, on = .(year, carbon_price_scenario), nomatch = 0]
  scenario_dt = scenario_dt[ccs_scens_all, on = .(year, ccs_scenario), nomatch = 0]
  scenario_dt = scenario_dt[setback_scens, on = .(doc_field_code, setback_scenario), nomatch = 0]
  scenario_dt = scenario_dt[prod_quota_scens, on = .(year, prod_quota_scenario), nomatch = 0]
  scenario_dt = scenario_dt[excise_tax_scens, on = .(year, excise_tax_scenario), nomatch = 0]
  
  ## compute tax
  scenario_dt[, tax := tax_rate * oil_price_usd_per_bbl]
  scenario_dt[, tax_rate:= NULL]
  ...
  setcolorder(scenario_dt, c('year', 'doc_field_code', 'doc_fieldname', 'oil_price_scenario', 'innovation_scenario', 'carbon_price_scenario', 'ccs_scenario',
