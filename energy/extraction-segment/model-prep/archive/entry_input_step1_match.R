...
field_field_m <- read_csv("/Volumes/GoogleDrive/Shared\ drives/emlab/projects/current-projects/calepa-cn/outputs/stocks-flows/entry-model-input/field_x_field_match.csv")
...
w_matches <- field_asset_well_match %>%
  rename(nn_field_code = FieldCode)

sp_matches2 <- sp_matches %>%
  left_join(w_matches) %>%
  select(FieldCode, original_asset_name, n_wells_asset, n_wells_asset, match_method, dist)
...
## save file
write_csv(all_combos, path = "/Volumes/GoogleDrive/Shared\ drives/emlab/projects/current-projects/calepa-cn/outputs/stocks-flows/entry-model-input/field_asset_matches_v2.csv")
