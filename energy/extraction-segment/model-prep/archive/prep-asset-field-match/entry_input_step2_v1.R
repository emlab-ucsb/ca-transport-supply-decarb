...
save_directory     <- "outputs/stocks-flows/entry-input-df/v1/"
...
## files
field_asset_match_file  <- "field_asset_matches_v1_revised.csv"
rystad_econ_file        <- "oil_asset_opex_capex_govtt_clean.csv"
rystad_entry_file       <- "rystad_entry_variables.csv"
rystad_imputed_file     <- "Rystad_cost_imputed_v1_revised.csv"
...
field_asset_matches_v1 <- fread(paste0(proj_dir, output_dir, "entry-model-input/", field_asset_match_file), colClasses = c('doc_field_code' = 'character'))
...
prod_df <- full_join(field_prod_df_all, field_asset_matches_v1) 

## View NA
prod_df %>% filter(is.na(original_asset_name)) %>% select(doc_field_code) %>% unique()
## Coal Oil Point offshore (154); 000 Any Field
...
prod_econ_df3 <- all_combos %>%
  filter(!doc_field_code %in% remove_doc) %>%
