...
# inputs ------
  ...
  para_dir      = "/Volumes/GoogleDrive/Shared drives/emlab/projects/current-projects/calepa-cn/outputs/decline-historic/parameters/"
  # yr_fil        = "fitted-parameters_field-vintage_yearly_entry_modeled_only.csv"
  yr_fil        = "exponential_fitted-parameters_field-vintage_yearly_entry_modeled_only.csv"
  prod_fil      = "/Volumes/GoogleDrive/Shared drives/emlab/projects/current-projects/calepa-cn/outputs/stocks-flows/crude_prod_x_field.csv"
...
# outputs -------
  ...
  save_fil      = "exponential_forecasted_decline_parameters_2020_2050.csv"
...
# read in data ------
  ...
  para_year = fread(paste0(para_dir, yr_fil))
  para_year[, no_wells.x := NULL]
  setnames(para_year, 'no_wells.y', 'no_wells')
  
  prod_2019 = fread(prod_fil, header = T)
...
# remove pre 1978 -----
  ...
  para_year_2 = para_year[! vintage == 'pre 1978']
...
# count unique vintages for each field -----
  ...
  para_year[, un_vintage := uniqueN(vintage), by = "doc_field_code"]
  para_year_2[, un_vintage := uniqueN(vintage), by = "doc_field_code"]
...
# years to predict -----
  ...
  pred_years = data.table(first_year = c(2013, seq(2020, 2045, 5)))
  
# remove outliers outside of 1st and 99th percentile ------
  
  para_clean = para_year_2[ b2 > quantile(b2, 0.01, na.rm = T) & b2 < quantile(b2, 0.99, na.rm = T)]
  para_clean = para_clean[ d2 > quantile(d2, 0.01, na.rm = T) & d2 < quantile(d2, 0.99, na.rm = T)]
...
# regress parameters for top 10 fields ------
  ...
  pred_top10 = para_top10[first_year %in% 1980:2019, .(q_i = predict(lm(q_i ~ first_year),  pred_years),
  ...
  pred_top10[, first_year := rep(pred_years[, first_year], uniqueN(pred_top10[, doc_field_code]))]
...
# take median of parameters across vintages -------
  ...
  median_params = para_clean[, .(median_q_i = median(q_i, na.rm = T),
                                 ...
                                 median_d2 = median(d2, na.rm = T),
                                 median_int = median(int_yr, na.rm = T)), by = .(first_year, vintage)]
  setorder(median_params, "first_year")
  
# regress median parameters ------
  
  pred_all = median_params[, .(q_i = predict(lm(median_q_i ~ first_year),  pred_years),
                               D = predict(lm(median_D ~ first_year),  pred_years),
                               b2 = predict(lm(median_b2 ~ first_year),  pred_years),
                               d2 = predict(lm(median_d2 ~ first_year),  pred_years),
                               int_year = predict(lm(median_int ~ first_year),  pred_years))]
  pred_all[, first_year := pred_years[, first_year]]
  
# for negative values of predicted intercept years, replace with median of 1990-20
  
  pred_all[int_year < 0, int_year := median(median_params[first_year %in% 1990:2010, median_int])]
  
# remove 2013 predictions -----
  
  pred_all = pred_all[! first_year == 2013]

# add end year of vintage ----
  
  pred_all[, end_year := first_year + 4]

# generate vintages -----
  
  pred_all[, vintage := paste(first_year, end_year, sep = "-")]

# duplicate rows for each year of vintage ----
  
  pred_all_full = pred_all[ , list(freq = rep(1,5)), by = c("q_i", "D", "b2", "d2", "int_year", "first_year", "end_year", "vintage")]
...
# export to csv -----
  ...
  fwrite(pred_all_full, paste0(save_dir, save_fil), row.names = F)
