...
compiled_ext_path  = paste0(main_path_external, 'academic-out/extraction_/')
...
field_out_list <- list()
  ...
  for (i in 1:length(subset_ids)) {
    ...
    field_out_tmp <- readRDS(paste0)
    saveRDS(field_out_tmp, paste0(field_save_path, id_name_tmp, "_field_results.rds"))
    
    county_out_tmp <- readRDS()
    saveRDS(county_out_tmp, paste0(county_save_path, id_name_tmp, "_county_results.rds"))
    
    state_out_tmp <- readRDS()
    saveRDS(state_out_tmp, paste0(state_save_path, id_name_tmp, "_state_results.rds"))
    
    field_out_list[[i]] <- field_out_tmp
     
  }

field_subset_all <- rbindlist(field_outlist)

fwrite(field_subset_all, paste0(field_save_path, id_name_tmp, "_field_results.rds"))
