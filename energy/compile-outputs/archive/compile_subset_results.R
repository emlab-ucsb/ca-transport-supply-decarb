## Tracey Mangin
## September 6, 2021
## compile and save subset scenarios
...
## read in from external or drive
ext_extract_folder = '_2021-09-07'
extraction_folder = '_2021-09-21'

compiled_ext_path  = paste0(main_path_external, 'academic-out/extraction', ext_extract_folder, '/')
compiled_path      = paste0(academic_out, '/extraction', extraction_folder, '/')
field_save_path     = paste0(compiled_path, 'field-results/')
state_save_path     = paste0(compiled_path, 'state-results/')
...
state_out_list <- list()
  ...
  for (i in 1:length(subset_ids)) {
    ...
    field_out_tmp <- readRDS(paste0(field_save_path, id_name_tmp, '_field_results.rds'))
    ...
    county_out_tmp <- readRDS(paste0(county_save_path, id_name_tmp, '_county_results.rds'))
    ...
    state_out_tmp <- readRDS(paste0(state_save_path, id_name_tmp, '_state_results.rds'))
...
fwrite(field_subset_all, paste0(field_save_path, "subset_field_results.csv"))
fwrite(county_subset_all, paste0(county_save_path, "subset_county_results.csv"))
fwrite(state_subset_all, paste0(state_save_path, "subset_state_results.csv"))
