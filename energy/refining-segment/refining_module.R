...
# outputs ---------
  ...
  save_path   = '/Volumes/GoogleDrive/Shared drives/emlab/projects/current-projects/calepa-cn/outputs/predict-production/refining_20201121/total_exports'
...
# plot theme & palettes ------
  ...
  names(pal_refinery) = unique(c(dt_refcap[, refinery_name], dt_renref[, refinery_name]))
...
# ------------------------- forecasted demand prep & analysis ------------------------- 
  ...
  # create time series of exports from 2019 levels to zero in 2045 ------
    ...
    # merge with 2019 exports
      ts_exports = refined_exports_annual[year == 2019][ts_exports, on = .(region, year, fuel)]
      
    # set 2045 exports to zero
      ts_exports[year == 2045, export_bbl := 0]

    # linearly interpolate within each group
      ts_exports_wide = dcast(ts_exports, region + year ~ fuel, value.var = 'export_bbl')
      setorder(ts_exports_wide, year, region)
      setcolorder(ts_exports_wide, c('region', 'year', 'gasoline', 'diesel', 'jet'))
      
      ts_exports_wide[, gasoline := ((0 - gasoline[year == 2019])/(2045-2019))*(year - 2045), by = .(region)]
      ts_exports_wide[, diesel := ((0 - diesel[year == 2019])/(2045-2019))*(year - 2045), by = .(region)]
      ts_exports_wide[, jet := ((0 - jet[year == 2019])/(2045-2019))*(year - 2045), by = .(region)]
      
    # get long form
      ts_exports = melt(ts_exports_wide, id.vars = c('region', 'year'), measure.vars = c('gasoline', 'diesel', 'jet'), 
...
# ------------------------- diagnostics ------------------------- 
  ...
  # GJD production at all refineries -- state level (area plot) --------
    ...
    # get historic exports
      agg_hist_exp = refined_exports_annual[, .(consumption_bbl = sum(abs(export_bbl), na.rm = T)), by = .(year, fuel)]
...
# -------------------- more plots  -------------------------
  ...
  # save outputs ----------
    ...
    fwrite(outputs_indiv, file.path(output_path, 'refining_scenario_outputs_refinery_total_exports.csv'), row.names = F)
    fwrite(outputs_clus, file.path(output_path, 'refining_scenario_outputs_cluster_total_exports.csv'), row.names = F)
    fwrite(outputs_state, file.path(output_path, 'refining_scenario_outputs_state_total_exports.csv'), row.names = F)
