...
map_fig_a <- ggdraw(fig1_map) +
  draw_plot(dac_legend, x = 0.2, y = 0.2, width = 0.05, height = 0.05) +
...
## counties boundaries
county_boundaries <- st_read(paste0(main_path, "data/GIS/raw/CA_Counties/CA_Counties_TIGER2016.shp")) %>% 
  st_transform(ca_crs) %>%
  dplyr::select(adj_county_name = NAME)
...
# map2 <- st_transform(map_ct,ca_crs)
field1_intersection2<-st_transform(field1_intersection,ca_crs)

mapRangeLim<-c(-114,-123,32.5,39)

total_pm25 <- ggplot () +
  geom_sf(data = ct_map, aes(fill=total_pm25), color=NA) + theme_void() + labs(fill=expression(paste("PM"[2.5], " (",mu,"/",m^3,")"))) +
  scale_fill_gradient(high = "#FF0000", low = "#FFFFFF", space = "Lab", 
                      limits = c(min(ct_map$total_pm25), max(ct_map$total_pm25))) +
  geom_sf(data = field1_intersection, fill="transparent", color="black") +
