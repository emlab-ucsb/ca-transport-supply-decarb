...
start_vals <- setback_carbon_out[, .(carbon_price_scenario, target_scen, ccs_scenario)]
...
setcolorder(carbon_px_scens_search, c("year", 'ccs_scenario', "carbon_price_scenario", "carbon_price", "units"))

carbon_px_scens_search[, target_scen := NULL]

## add id
carbon_px_scens_search[, id := paste0(carbon_price_scenario, "-", ccs_scenario)]
...
carbon_px_scens_search <- carbon_px_scens_search %>%
  mutate(carbon_price_scenario = as.character(carbon_price_scenario),
         units = as.character(units)) %>%
  group_by(ccs_scenario, carbon_price_scenario) %>%
  mutate(tval = row_number() - 1) %>%
  fill(carbon_price) %>%
  ungroup() %>%
  mutate(carbon_price = ifelse(tval == 0, carbon_price, calculate_val(x0 = carbon_price, r = perc_inc, t = tval))) %>%
  select(year, ccs_scenario, carbon_price_scenario, carbon_price, units)

## cobmine ccs_scenario with carbon price scenario
carbon_px_scens_search <- carbon_px_scens_search %>%
  mutate(carbon_price_scenario = paste(carbon_price_scenario, ccs_scenario, sep = "-")) %>%
  select(-ccs_scenario)
